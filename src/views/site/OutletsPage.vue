...
async function loadOutlets() {
    ...
    error.value = ''
    try {
        outlets.value = await apiGet('/outlets/')
    } catch (e) {
        error.value = 'Ошибка загрузки аутлетов'
        outlets.value = []
    } finally {
        loading.value = false
    }
...
const createForm = ref({ name: '', description: '', discount_percent: null, start_date: '', end_date: '', is_active: true })
async function onCreate() {
    try {
        await apiJson('/outlets/', 'POST', {
            name: createForm.value.name,
            description: createForm.value.description || null,
            discount_percent: createForm.value.discount_percent != null ? Number(createForm.value.discount_percent) : null,
            start_date: createForm.value.start_date ? formatDateForAPI(createForm.value.start_date) : null,
            end_date: createForm.value.end_date ? formatDateForAPI(createForm.value.end_date) : null,
            is_active: !!createForm.value.is_active,
        })
        showCreate.value = false
        createForm.value = { name: '', description: '', discount_percent: null, start_date: '', end_date: '', is_active: true }
        await loadOutlets()
    } catch (e) {
        alert('Ошибка при создании аутлета')
    }
}

// Edit modal
const showEdit = ref(false)
const editForm = ref({ id: null, name: '', description: '', discount_percent: null, start_date: '', end_date: '', is_active: false })
function openEdit(d) {
    editForm.value = {
        id: d.id,
        name: d.name || '',
...
async function onUpdate() {
    try {
        await apiJson(`/outlets/${editForm.value.id}`, 'PUT', {
            id: editForm.value.id,
            name: editForm.value.name,
            description: editForm.value.description,
            discount_percent: editForm.value.discount_percent != null ? Number(editForm.value.discount_percent) : null,
            start_date: editForm.value.start_date ? formatDateForAPI(editForm.value.start_date) : null,
            end_date: editForm.value.end_date ? formatDateForAPI(editForm.value.end_date) : null,
            is_active: !!editForm.value.is_active,
        })
        showEdit.value = false
        await loadOutlets()
    } catch (e) {
        alert('Ошибка при сохранении аутлета')
    }
}
...
async function onDelete() {
    try {
        await apiJson(`/outlets/${deleteId.value}`, 'DELETE', null)
        showDelete.value = false
        await loadOutlets()
    } catch (e) {
        alert('Ошибка при удалении аутлета')
    }
}
...
async function openProducts(outletId) {
    currentOutletId.value = outletId
    showProducts.value = true
    await fetchOutletProducts(outletId)
}

async function fetchOutletProducts(outletId) {
    try {
        outletProducts.value = await apiGet(`/outlets/${outletId}/products`)
    } catch (e) {
        outletProducts.value = []
        alert('Ошибка загрузки товаров аутлета')
    }
}

async function removeProductFromOutlet(productId) {
    try {
        await apiJson(`/outlets/${currentOutletId.value}/products`, 'DELETE', { product_ids: [productId] })
        await fetchOutletProducts(currentOutletId.value)
    } catch (e) {
...
const modalHasImage = ref('')
const modalSortField = ref('id')
const modalSortDir = ref('desc')
const modalCanLoadMore = ref(true)

const outletProductIds = computed(() => new Set(outletProducts.value.map(p => p.good_id)))
...
async function addProductToOutlet(productId) {
    if (outletProductIds.value.has(productId)) {
        alert('Товар уже в этом аутлете')
        return
    }
    try {
        await apiJson(`/outlets/${currentOutletId.value}/products`, 'POST', { product_ids: [productId] })
        await fetchOutletProducts(currentOutletId.value)
...
<template>
    <div class="container mt-4">
        <h1>Outlet</h1>
        <button class="btn btn.success mb-3 btn-success" @click="showCreate = true">Добавить аутлет</button>
        ...
        <table v-else class="table">
            ...
            <tbody>
                <tr v-for="d in outlets" :key="d.id">
                    <td>{{ d.name }}</td>
                    <td>{{ d.description }}</td>
                    <td>{{ d.discount_percent ?? '' }}</td>
                    <td>{{ d.start_date ? new Date(d.start_date).toLocaleString('ru-RU') : '' }}</td>
                    <td>{{ d.end_date ? new Date(d.end_date).toLocaleString('ru-RU') : '' }}</td>
                    <td>{{ d.is_active ? 'Активен' : 'Неактивен' }}</td>
                    <td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Новый аутлет</h5>
                    ...
                    <div class="modal-body">
                        <form @submit.prevent="onCreate">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea v-model="createForm.description" class="form-control"></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Скидка (%)</label>
                                <input v-model.number="createForm.discount_percent" type="number" min="0" max="100" class="form-control" />
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Дата начала</label>
                                <input v-model="createForm.start_date" type="datetime-local" class="form-control" />
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Дата окончания</label>
                                <input v-model="createForm.end_date" type="datetime-local" class="form-control" />
                            </div>
                            <div class="form-check mb-3">
                                <input v-model="createForm.is_active" class="form-check-input" type="checkbox" id="createIsActive">
                                <label class="form-check-label" for="createIsActive">Активен</label>
                            </div>
                        </form>
                    </div>
        ...
        <div v-if="showEdit" class="modal fade show d-block" tabindex="-1">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Редактировать аутлет</h5>
                    ...
                    <div class="modal-body">
                        <form @submit.prevent="onUpdate">
                            ...
                            <div class="mb-3">
                                <label class="form-label">Скидка (%)</label>
                                <input v-model.number="editForm.discount_percent" type="number" min="0" max="100" class="form-control" />
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Дата начала</label>
        ...
        <div v-if="showDelete" class="modal fade show d-block" tabindex="-1">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">Вы уверены, что хотите удалить этот аутлет?</div>
                    <div class="modal-footer">
        ...
        <div v-if="showProducts" class="modal fade show d-block" tabindex="-1">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Товары аутлета</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div v-if="outletProducts.length === 0" class="alert alert-info">Нет товаров</div>
                        <div v-else>
                            <div v-for="p in outletProducts" :key="p.good_id" class="card mb-2 product-row px-3">
                                <div class="card-body row align-items-start g-0 mb-0">
                                    ...
                                    <div class="col-12 col-md-2 text-end">
                                        ...
                                        <button class="btn btn-danger btn-sm ms-2" @click="removeProductFromOutlet(p.good_id)">-</button>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div v-if="showAddProducts" class="modal fade show d-block" tabindex="-1">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Добавить товары в аутлет</h5>
                                    ...
                                    <table class="table align-middle mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            <tr v-else v-for="p in filteredSortedModalProducts" :key="p.good_id">
                                                ...
                                                <td class="align-middle">
                                                    <button class="btn btn-success btn-sm" :disabled="outletProductIds.has(p.good_id)" @click="addProductToOutlet(p.good_id)">+</button>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
